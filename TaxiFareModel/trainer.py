...
class Trainer(object):
    ...
    def set_pipeline(self):
        ...
        feateng_steps = self.kwargs.get(
            "feateng", ["distance", "time_features", "direction", "distance_to_center"]
        )
        if memory:
            memory = mkdtemp()
        ...
        pipe_distance = make_pipeline(
            DistanceTransformer(distance_type=dist, **DIST_ARGS), RobustScaler()
        )
        ...
        if self.optimize:
            self.pipeline.steps.insert(
                -1, ["optimize_size", OptimizeSize(verbose=False)]
            )
